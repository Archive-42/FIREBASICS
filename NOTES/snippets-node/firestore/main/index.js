...
async function arrayFilter(db) {
  ...
  const westCoastCities = citiesRef
...
async function arrayContainsAnyQueries(db) {
  ...
  const coastalCities = await citiesRef
...
async function inQueries(db) {
  ...
  const notUsaOrJapan = await citiesRef
    .where("country", "not-in", ["USA", "Japan"])
    .get();
  // [END firestore_query_filter_not_in]

  // [START firestore_query_filter_in_with_array]
  const exactlyOneCoast = await citiesRef
    .where("region", "in", [["west_coast", "east_coast"]])
    .get();
...
async function orderAndLimit(db) {
  ...
  const byStateByPopRes = await citiesRef
    .orderBy("state")
  ...
  const biggestRes = await citiesRef
...
async function multipleCursorConditions(db) {
  ...
  const startAtNameAndStateRes = await db
    .collection("cities")
    .orderBy("name")
    .orderBy("state")
